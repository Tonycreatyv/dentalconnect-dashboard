...
export function interpretInsuranceTurn(args: {
  ...
  const selectedService = shouldParseType
    ? resolveInsuranceServiceOption(args.inboundText, args.services)
    : null;
  ...
  const fields = {
    ...
    nombre: nextExpected === "insurance_name" ? extractName(args.inboundText) : null,
    estado: nextExpected === "insurance_location" ? extractState(args.inboundText) : null,
    telefono: null,
    email: nextExpected === "insurance_email" ? extractEmail(args.inboundText) : null,
    seguro_actual: nextExpected === "insurance_current" ? extractCurrentInsurance(args.inboundText) : null,
    presupuesto: nextExpected === "insurance_budget" ? extractBudget(args.inboundText) : null,
    horario_preferido: nextExpected === "insurance_preferred_time" ? extractPreferredTime(args.inboundText) : null,
  };
  const mergedContact = {
