...
function database(entry: Record<string, unknown>, overrides: Rows = {}) {
  ...
  const from = (table: string) => {
    const query: Record<string, unknown> & PromiseLike<unknown> = {
      then(resolve, reject) {
        return Promise.resolve({ data: rows[table] ?? null, error: null }).then(
          resolve,
          reject,
        );
      },
...
function entry(
  type: "general" | "service" | "campaign" | "location",
  ...
  return {
    ...
    attribution_label: "Volante agosto",
    attribution_source: "community_flyer",
    ...context,
  };
}
  ...
  for (
    const candidate of [
      ...
      entry("location", {
        ...
        service_id: "luis_compra_super",
      }),
    ]
  ) {
    const resolved = await resolveReferralQrEntry(
      database(candidate) as any,
      code,
    );
    assert(resolved);
    ...
    assertStringIncludes(
      ...
      "https://wa.me/14045551212",
    );
    assertEquals(
      JSON.stringify(publicResult).includes("organizationId"),
      false,
    );
...
Deno.test("supermarket QR prefill enters the canonical coupon intent and preserves opaque attribution", async () => {
  const resolved = await resolveReferralQrEntry(
    database(entry("campaign", { campaign_key: "mi_tierra_10" })) as any,
    code,
  );
  assert(resolved);
  assertEquals(
    qrWhatsAppPrefill(resolved),
    `Quiero mi cupón de supermercado [[rhq:${code}]]`,
  );
});

Deno.test("medical service QR prefill enters the canonical medical coupon flow", async () => {
  const resolved = await resolveReferralQrEntry(
    database(entry("service", { service_id: "luis_cupon_medico" }), {
      service_configs: {
  ...
  assertEquals(
    qrWhatsAppPrefill(resolved),
    `Quiero mi cupón médico [[rhq:${code}]]`,
  );
  const result = await handleReferralHubTurn({
    supabase: database(entry("service", { service_id: "luis_cupon_medico" }), {
      service_configs: {
        id: "luis_cupon_medico",
        organization_id: organizationId,
        nombre: "Cupón médico",
    ...
    leadId: "lead-id",
    leadState: null,
    inboundText: `Quiero mi cupón médico [[rhq:${code}]]`,
    channel: "whatsapp",
  });
  assertEquals(
    result.debugNote,
    "referral_hub:persistent_coupon:whatsapp:luis_cupon_medico",
  );
  assertStringIncludes(
    (result.outboundMessages?.[0] as any).text,
    "Código: RH-LG-12345",
  );
  assertEquals((result.outboundMessages?.[1] as any).type, "image");
  assertEquals(result.reply, "¿Qué quieres hacer ahora?");
});

Deno.test("controlled demo QR fallback never replaces a canonical connected destination", async () => {
  const disabled = await resolveReferralQrEntry(
    database(entry("service", { service_id: "luis_cupon_medico" }), {
      service_configs: {
        id: "luis_cupon_medico",
        organization_id: organizationId,
  ...
  assert(disabled);
  assertEquals(controlledDemoQrPhone(disabled, true), "15551807992");
  assertEquals(controlledDemoQrPhone(disabled, false), null);

  const connected = { ...disabled, whatsappPhone: "17707137058" };
  assertEquals(controlledDemoQrPhone(connected, true), null);
  assertStringIncludes(
    String(publicReferralQrResolution(connected).whatsapp_url),
    "wa.me/17707137058",
  );
});
...
Deno.test("QR resolver rejects unknown, inactive, expired, and cross-organization context", async () => {
  assertEquals(
    await resolveReferralQrEntry(
      database(entry("general"), { referral_qr_entries: null }) as any,
      code,
    ),
    null,
  );
  assertEquals(
    await resolveReferralQrEntry(
      database(entry("general", { active: false })) as any,
      code,
    ),
    null,
  );
  assertEquals(
    await resolveReferralQrEntry(
      database(entry("general", { expires_at: "2026-01-01T00:00:00Z" })) as any,
      code,
    ),
    null,
  );
  assertEquals(
    await resolveReferralQrEntry(
      database(entry("service", { service_id: "luis_compra_super" }), {
        service_configs: {
          id: "luis_compra_super",
...
Deno.test("opaque QR marker ignores forged service text and enters the canonical grocery flow", async () => {
  ...
  const result = await handleReferralHubTurn({
    ...
    leadId: "lead-id",
    leadState: null,
    inboundText: `Hola [[rhq:${code}]] referral_service:luis_accidente`,
    channel: "whatsapp",
  });
  ...
  assertEquals(
    (result.leadPatch?.extracted_data as any).qr_entry.public_code,
    code,
  );
...
Deno.test("campaign QR fulfills the stored coupon before the existing grocery upsell", async () => {
  ...
  assertEquals(
    (result.leadPatch?.extracted_data as any).qr_entry.campaign_key,
    "mi_tierra_10",
  );
  ...
  assertEquals(
    ((baskets.statePatch.collected as any).referral_hub.grocery as any)
      .sourceCampaign,
    "mi_tierra_10",
  );
});
...
Deno.test("location QR keeps the durable grocery ZIP validation intact", async () => {
  ...
  const entryResult = await handleReferralHubTurn({
    supabase: db as any,
    organizationId,
    leadId: "lead-id",
    leadState: null,
    inboundText: `[[rhq:${code}]]`,
    channel: "whatsapp",
  });
  const baskets = await handleReferralHubTurn({
    supabase: db as any,
    organizationId,
    leadId: "lead-id",
    leadState: entryResult.statePatch,
    inboundText: "",
    payloadAction: "referral_grocery:baskets",
    channel: "whatsapp",
  });
  const invalidZip = await handleReferralHubTurn({
    supabase: db as any,
    organizationId,
    leadId: "lead-id",
    channelUserId: "+14045551212",
    leadState: baskets.statePatch,
    inboundText: "not a zip",
    channel: "whatsapp",
  });
...
Deno.test("QR marker parser accepts only the opaque marker", () => {
  ...
  assertEquals(
    extractReferralQrPublicCode(
      "campaign=mi_tierra_10&service=luis_compra_super",
    ),
    null,
  );
});
...
Deno.test("QR migration keeps the registry private and constrained", async () => {
  const sql = await Deno.readTextFile(
    new URL(
      "../../../migrations/20260808000100_referral_qr_entries.sql",
      import.meta.url,
    ),
  );
  ...
  assertStringIncludes(
    sql,
    "entry_type in ('general', 'service', 'campaign', 'location')",
  );
