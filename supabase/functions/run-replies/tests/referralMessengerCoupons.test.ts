...
const assets = Object.fromEntries(
  Object.entries(urls).map(([serviceId, imageUrl]) => [
    serviceId,
    {
...
function referralState(result: { statePatch: Record<string, unknown> }) {
  return ((result.statePatch.collected as any)?.referral_hub ?? {}) as Record<
    string,
    unknown
  >;
}
...
Deno.test("supermarket coupon orders code text, native image, then actions", async () => {
  const result = await coupon("luis_cupon_super");
  assertEquals(result.outboundMessages, [
    {
      type: "text",
      text:
        "¡Claro! Aquí tienes tu cupón de supermercado.\n\nCódigo: LG-COUPON-123\n\nPreséntalo al momento de pagar en un comercio participante.",
    },
    {
      type: "image",
...
Deno.test("coupon continuation routes to services without reissuing a coupon", async () => {
  ...
  assertEquals(result.interactiveButtons?.length, 6);
});
...
Deno.test("medical and dental coupons select their exact HTTPS images", async () => {
  ...
  assertEquals(
    (medical.outboundMessages?.[1] as any).url,
    urls.luis_cupon_medico,
  );
  assertEquals(
    (dental.outboundMessages?.[1] as any).url,
    urls.luis_cupon_dental,
  );
...
Deno.test("production coupon assets are active and map to exact canonical URLs", async () => {
  ...
  assertEquals(
    REFERRAL_HUB_COUPON_ASSETS.luis_cupon_medico.image_url,
    "https://referral.creatyv.io/images/coupons/lg-medical-coupon.jpeg",
  );
  assertEquals(
    REFERRAL_HUB_COUPON_ASSETS.luis_cupon_medico.intro_text,
    "Tu beneficio está listo ✅",
  );
  assertEquals(
    REFERRAL_HUB_COUPON_ASSETS.luis_cupon_medico.benefit_text,
    "20% de descuento en servicios médicos participantes.",
  );
  assertEquals(
    REFERRAL_HUB_COUPON_ASSETS.luis_cupon_medico.instructions,
    "Muéstralo al llegar a una clínica participante.",
  );
  assertEquals(REFERRAL_HUB_COUPON_ASSETS.luis_cupon_medico.active, true);
  assertEquals(
    REFERRAL_HUB_COUPON_ASSETS.luis_cupon_super.image_url,
  ...
  assertEquals(
    REFERRAL_HUB_COUPON_ASSETS.luis_cupon_super.campaign_key,
    "mi_tierra_10",
  );
  assertEquals(
    REFERRAL_HUB_COUPON_ASSETS.luis_cupon_medico.campaign_key,
    "medico_urgencias_20",
  );
...
Deno.test("production conditions resolve all exact service IDs with empty integrations", async () => {
  for (
    ...
    assertEquals(
      result.debugNote,
      `referral_hub:persistent_coupon:messenger:${serviceId}`,
    );
...
Deno.test("WhatsApp coupon uses persistent issuance and prepared delivery tracking", async () => {
  ...
  assertEquals(
    result.debugNote,
    "referral_hub:persistent_coupon:whatsapp:luis_cupon_dental",
  );
...
Deno.test("coupon sequence records stage failures and sent only after provider acceptance", async () => {
  ...
  const providerSuccessIndex = source.indexOf(
    "const outboundProviderMessageId",
  );
...
Deno.test("Messenger token loading remains organization-scoped with no global Page-token fallback", async () => {
  ...
  assertEquals(
    source.includes('Deno.env.get("META_PAGE_ACCESS_TOKEN")'),
    false,
  );
