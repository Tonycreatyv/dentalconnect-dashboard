...
export default function CouponScanner({ onScan }: { onScan: (value: string) => void }) {
  ...
  return <div className="space-y-3"><button type="button" onClick={()=>{setError("");setActive(true);}} className="hub-primary">Abrir cámara</button>{active?<video ref={videoRef} className="aspect-square w-full rounded-2xl bg-black object-cover" muted playsInline />:null}{error?<p className="text-sm text-amber-700">{error}</p>:null}</div>;
}
