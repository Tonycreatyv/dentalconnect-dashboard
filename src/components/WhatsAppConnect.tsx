...
export default function WhatsAppConnect(
  ...
  const loadStatus = useCallback(async () => {
    const result = await supabase.from("org_settings").select(
      "whatsapp_enabled,whatsapp_phone_number,whatsapp_display_name,whatsapp_phone_number_id,whatsapp_registered,whatsapp_webhooks_subscribed,whatsapp_token_expires_at",
  ...
  const exchange = useCallback(
    async (
      ...
      const result = await supabase.functions.invoke("whatsapp-signup", {
        body: {
          action: "exchange",
          code: input.code,
          state: input.state,
